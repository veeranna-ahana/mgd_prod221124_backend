...
shiftManagerService.get(
	...
	async (req, res, next) => {
        ...
        JOIN machine_data.magod_process_list p On p.ProcessDescription=n.Operation where
         m.Working and p.Service = 1 OR p.Service = -1 and m.Working`,
				async (err, data) => {
					if (err) logger.error(err);
					const machineMap = {};
...
shiftManagerService.post(
	"/shiftManagerServiceFilteredMachines",
	jsonParser,
	async (req, res, next) => {
...
shiftManagerService.get(
	...
	async (req, res, next) => {
		...
		try {
			mchQueryMod(
				`SELECT ol.Operation, ol.ProcessId,mpl.Machine_srl, ml.refName, n.NCProgramNo, n.TaskNo , n.PStatus FROM machine_data.service_operationslist pcol
        ...
        LEFT JOIN magodmis.ncprograms n ON n.Machine=ml.refName AND n.Operation=ol.Operation AND (PStatus='Completed' || PStatus='Cutting' || PStatus='Processing')`,
				async (err, data) => {
					if (err) logger.error(err);
...
shiftManagerService.get(
	...
	async (req, res, next) => {
		...
		try {
			misQueryMod(
				`SELECT Cust_name, Cust_Code FROM magodmis.cust_data where LastBilling > '2021-06-11 00:00:00'`,
				async (err, data) => {
					if (err) logger.error(err);
					...
					for (let i = 0; i < data.length; i++) {
						try {
							...
							misQueryMod(
								`select NCProgramNo, TaskNo , Machine , PStatus from magodmis.ncprograms where Cust_Code = '${data[i].Cust_Code}' && (PStatus='Completed' || PStatus='Cutting' || PStatus='Processing')`,
								async (err, datanc) => {
									if (err) logger.error(err);
