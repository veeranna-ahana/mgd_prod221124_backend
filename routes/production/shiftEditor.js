...
shiftEditor.get("/shiftTimings", async (req, res, next) => {
  try {
    ...
    productionQueryMod(
      "Select * from magod_production.shiftdb",
      (err, data) => {
...
shiftEditor.get("/shiftInchargeList", async (req, res, next) => {
  ...
  try {
    const shiftInchargeNames = [];
    productionQueryMod(
      `Select Name from magod_production.shift_ic_list where Active='1'`,
      (err, data) => {
        if (err) logger.error(err);
        for (let i = 0; i < data.length; i++) {
          shiftInchargeNames[i] = data[i].Name;
...
shiftEditor.post(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      misQueryMod(
        `SELECT * FROM day_shiftregister WHERE ShiftDate='${finalDay}'`,
        (err, data) => {
...
shiftEditor.post(
  ...
  async (req, res, next) => {
    ...
    if (req.body === "") {
      ...
      try {
        misQueryMod(
          ` SELECT * FROM day_shiftregister WHERE ShiftDate='${newDates[0]}' || ShiftDate='${newDates[1]}' || ShiftDate='${newDates[2]}' || ShiftDate='${newDates[3]}' || ShiftDate='${newDates[4]}' || ShiftDate='${newDates[5]}' || ShiftDate='${newDates[6]}'`,
          (err, data) => {
            if (err) logger.error(err);
...
shiftEditor.post("/getDailyShiftPlan", jsonParser, async (req, res, next) => {
  try {
    misQueryMod(
      ` SELECT * FROM day_shiftregister WHERE ShiftDate='${req.body.ShiftDate}' and Shift='${req.body.Shift}'`,
      (err, data) => {
        if (err) logger.error(err);
...
shiftEditor.post(
  ...
  async (req, res, next) => {
    ...
    if (req.body.hasOwnProperty("DayShiftId")) {
      ...
      ToTimeNew =
        ToTime1[2] + "-" + ToTime1[1] + "-" + ToTime1[0] + " " + ToTime[1];
      // console.log("finalDay",finalDay,"FromTimeNew is",FromTimeNew,"ToTimeNew",ToTimeNew);

      try {
        misQueryMod(
          `SELECT * FROM magodmis.shiftregister where ShiftDate ='${finalDay}' and  Shift='${req.body.Shift}' and FromTime='${FromTimeNew}' and ToTime='${ToTimeNew}' ORDER BY ShiftID DESC`,
          (err, data) => {
            if (err) logger.error(err);
...
shiftEditor.post(
  ...
  async (req, res, next) => {
    ...
    let finalDayToTime =
      yearSplit1[0] + "-" + monthToTime + "-" + dayToTime + " " + yearSplit1[1];
    ...
    try {
      ...
      const [operatorData, machineData] = await Promise.all([
        new Promise((resolve, reject) => {
          misQueryMod(
            `SELECT * FROM magodmis.shiftregister WHERE ShiftDate='${req.body.ShiftDate}' AND (Shift='${req.body.Shift}' OR Shift='${newShift}' OR Shift='${newShift1}') AND Operator='${req.body.Operator}'`,
            (err, data) => {
              if (err) {
                console.error(err);
                return reject(err);
        ...
        new Promise((resolve, reject) => {
          misQueryMod(
            `SELECT * FROM magodmis.shiftregister WHERE ShiftDate='${req.body.ShiftDate}'AND (Shift='${req.body.Shift}' OR Shift='${newShift}' OR Shift='${newShift1}') AND Machine='${req.body.Machine}'`,
            (err, data) => {
              if (err) {
                console.error(err);
                return reject(err);
      ...
      if (
        ...
        misQueryMod(
          `INSERT INTO magodmis.shiftregister (ShiftDate, Shift, FromTime, ToTime, Machine, Operator, DayShiftID) VALUES 
                ('${req.body.ShiftDate}', '${req.body.Shift}', '${req.body.FromTime}', '${req.body.ToTime}', '${req.body.Machine}', '${req.body.Operator}', '${req.body.DayShiftID}')`,
          (err, data) => {
            if (err) {
              console.error(err);
              return next(err);
...
shiftEditor.post(
  ...
  async (req, res, next) => {
    ...
    try {
      misQueryMod(
        `DELETE FROM magodmis.shiftregister WHERE ShiftDate='${req.body.ShiftDate}' && Shift='${req.body.Shift}' && Machine='${req.body.Machine}' && Operator='${req.body.Operator}' and FromTime='${req.body.FromTime}' and ToTime='${req.body.ToTime}'`,
        (err, data) => {
          if (err) logger.error(err);
          //  console.log(data)
          res.send(data);
...
shiftEditor.post(
  "/updateSingleDaySihiftIncharge",
  jsonParser,
  async (req, res, next) => {
    // console.log('/updateSingleDaySihiftIncharge REQUEST' , req.body)
    try {
      misQueryMod(
        ` UPDATE  magodmis.day_shiftregister 
        SET Shift_Ic = '${req.body.newShift_Ic}'
        WHERE DayShiftId='${req.body.DayShiftId}'`,
        (err, data) => {
          if (err) logger.error(err);
          // console.log(data)
          res.send(data);
...
shiftEditor.post(
  "/updateSingleDaySihiftInstructions",
  jsonParser,
  async (req, res, next) => {
...
shiftEditor.post(
  ...
  async (req, res, next) => {
    ...
    for (let i = 0; i < letinputArray.length; i++) {
      try {
        misQueryMod(
          ` DELETE FROM magodmis.shiftregister WHERE Shift='${req.body.selectedShift}' && ShiftDate='${letinputArray[i]}' && Machine='${req.body.selectedMachine}' && Operator='${req.body.selectedOperator}' `,
          (err, data) => {
            if (err) logger.error(err);
            //console.log(data)
            //res.send(data)
...
shiftEditor.post("/deleteWeekShift", jsonParser, async (req, res, next) => {
  ...
  for (let i = 0; i < letinputArray.length; i++) {
    try {
      misQueryMod(
        ` DELETE FROM magodmis.shiftregister WHERE Shift='${
          req.body.selectedShift
        }' && ShiftDate='${letinputArray[i] + " 00:00:00"}'  `,
        (err, data) => {
          if (err) logger.error(err);
          // console.log(data)
          //res.send(data)
    ...
    try {
      misQueryMod(
        ` DELETE FROM magodmis.day_shiftregister WHERE Shift='${req.body.selectedShift}' && ShiftDate = '${letinputArray[i]}'`,
        (err, data) => {
          if (err) logger.error(err);
          //console.log(data)
          //res.send(data)
...
shiftEditor.post(
  "/deleteSingleDayShift",
  jsonParser,
  async (req, res, next) => {
    //console.log('/deleteSingleDayShift REQUEST' , req.body)
    try {
      misQueryMod(
        ` DELETE FROM magodmis.day_shiftregister WHERE DayShiftId='${req.body.DayShiftId}'`,
        (err, data) => {
          if (err) logger.error(err);
          //console.log(data)
          res.send(data);
...
shiftEditor.post("/setMachineOperators", jsonParser, async (req, res, next) => {
    ...
    try {
      ...
      if (operatorData.length > 0 || machineData.length > 0) {
        ...
        const insertData = await new Promise((resolve, reject) => {
          misQueryMod(
            `INSERT INTO magodmis.shiftregister (ShiftDate, Shift, FromTime, ToTime, Machine, Operator, DayShiftID) VALUES 
                        ('${inputArray[i].ShiftDate}', '${inputArray[i].Shift}', '${inputArray[i].FromTime}', '${inputArray[i].ToTime}', '${inputArray[i].Machine}', '${inputArray[i].Operator}', '${dayShiftID}')`,
            (err, data) => {
              if (err) {
                console.error(err);
                hasError = true;
  ...
  if (hasError) {
    return res
      .status(404)
      .send("No data found for the provided shift date and shift.");
  } else {
    ...
    try {
      ...
      for (let i = 0; i < inputArray.length; i++) {
        ...
        const dayShiftData = await new Promise((resolve, reject) => {
          misQueryMod(
            `select * FROM magodmis.day_shiftregister WHERE ShiftDate='${finalDay}'`,
            (err, data) => {
              if (err) reject(err);
              else resolve(data);
            }
        ...
        for (const dayShift of dayShiftData) {
          ...
          const shiftData = await new Promise((resolve, reject) => {
            misQueryMod(
              `select * FROM magodmis.shiftregister WHERE DayShiftID='${dayShift.ShiftId}' && Shift='${dayShift.Shift}'`,
              (err, data) => {
                if (err) reject(err);
                else resolve(data);
              }
...
shiftEditor.post(
  ...
  async (req, res, next) => {
    ...
    try {
      misQueryMod(
        ` SELECT * FROM magodmis.shiftregister WHERE ShiftDate='${newDates[0]}' || ShiftDate='${newDates[1]}' || ShiftDate='${newDates[2]}' || ShiftDate='${newDates[3]}' || ShiftDate='${newDates[4]}' || ShiftDate='${newDates[5]}' || ShiftDate='${newDates[6]}'`,
        (err, data) => {
          if (err) logger.error(err);
...
shiftEditor.post("/TryWeeklyPdf", jsonParser, async (req, res, next) => {
  try {
    const flatShiftArray = await Promise.all(
      req.body.ShiftDate.flatMap(async (date) => {
        ...
        try {
          ...
          return await Promise.all(
            firstQueryResult.map(async (item) => {
              const customObject = {
                ShiftIc: "",
                Shift: "",
                day: "",
              ...
              try {
                ...
                const subArrayData = await fetchSubArrayData(
                  item.Shift,
                  item.DayShiftId,
                  finalDay1
                );
    ...
    const sortedShiftArray = flatShiftArray.sort((a, b) =>
...
async function fetchSubArrayData(shift, dayShiftID, finalDay1) {
  ...
  return new Promise((resolve, reject) => {
    ...
    misQueryMod(
      `SELECT * FROM magodmis.shiftregister  WHERE Shift='${shift}' AND ShiftDate='${finalDay1}'`,
      (err, data) => {
...
shiftEditor.get("/getMachineList", async (req, res, next) => {
  try {
    const shiftInchargeNames = [];
    productionQueryMod(
      `Select * from machine_data.machine_list where activeMachine=1 and Working=1`,
      (err, data) => {
        if (err) logger.error(err);
        // for (let i = 0; i < data.length; i++) {
        //     shiftInchargeNames[i] = data[i].Name
...
shiftEditor.post("/printdayShiftPlan", jsonParser, (req, res, next) => {
  ...
  try {
    productionQueryMod(
      `SELECT * FROM magodmis.day_shiftregister WHERE ShiftDate='${shiftDate}'`,
      (err, dayShiftData) => {
        if (err) {
          console.log("err", err);
        ...
        productionQueryMod(
          `SELECT * FROM magodmis.shiftregister WHERE ShiftDate='${newDate}'`,
          (err, shiftData) => {
            if (err) {
              console.log("err", err);
              next(err); // Forward error to error handler middleware
            ...
            const newData = dayShiftData.map((dayShift) => {
              ...
              const shiftOperators = shiftData.filter(
                (operator) =>
                  operator.Shift.trim().toLowerCase() ===
        ...
        misQueryMod(
          ...
          (err, data) => {
            ...
            if (data.length > 0) {
              ...
              if (
                shifts.includes("First") &&
                shifts.includes("Second") &&
                shifts.includes("Third")
              ) {
