...
shiftManagerProfile.post(
	...
	async (req, res, next) => {
		...
		try {
			misQueryMod(
				`Select * from magodmis.day_shiftregister where ShiftDate = '${req.body.ShiftDate}' && Shift = '${req.body.Shift}'`,
				(err, data) => {
...
shiftManagerProfile.get("/profileListMachines", async (req, res, next) => {
	...
	try {
		misQueryMod(
			`SELECT distinct m.refName , m.Machine_srl FROM machine_data.machine_list m,
        machine_data.machine_process_list m1,machine_data.operationslist o,
         ...
         AND m.Working AND p.OperationId=o.OperationID`,
			async (err, data) => {
				if (err) logger.error(err);
				...
				for (let i = 0; i < data.length; i++) {
					...
					try {
						mchQueryMod(
							`select RefProcess from machine_process_list where Machine_srl='${data[i].Machine_srl}'`,
							(err, datanew) => {
								if (err) logger.error(err);
...
shiftManagerProfile.get(
	"/profileListMachinesTaskNo",
	async (req, res, next) => {
		// console.log('OnClick of Machines')
		let outputArray = [];
		try {
			misQueryMod(
				`SELECT DISTINCT m.refName , m.Machine_srl, n.TaskNo, n.Mtrl_Code,n.NCProgramNo, n.PStatus,n.Operation 
        ...
        JOIN machine_data.magod_process_list p On p.ProcessDescription=n.Operation where
         m.Working and p.Profile = 1 OR p.Profile = -1 and m.Working`,
				async (err, data) => {
					if (err) logger.error(err);
					// console.log('data length is ' , data.length)
...
shiftManagerProfile.post(
	"/shiftManagerProfileFilteredMachines",
	jsonParser,
	async (req, res, next) => {
        ...
        machine_data.machine_process_list m1,machine_data.operationslist o,
         machine_data.profile_cuttingoperationslist p 
         WHERE m1.Machine_srl=m.Machine_srl AND o.Operation=m1.RefProcess 
         AND m.Working AND p.OperationId=o.OperationID AND m1.Mprocess='${req.body.Operation}'`,
				(err, data) => {
					if (err) logger.error(err);
					// console.log(data)
					res.send(data);
...
WHERE (n.PStatus = 'Cutting' OR n.PStatus = 'Processing')
    AND n.NCProgramNo = '${req.body.NCProgramNo}'
    AND EXISTS (
        SELECT 1 
        FROM machine_data.magod_process_list AS p 
        WHERE p.ProcessDescription = n.Operation 
...
shiftManagerProfile.post(
	"/profileListMachinesProgramesCompleted",
	jsonParser,
	async (req, res, next) => {
		try {
			mchQueryMod(
...
shiftManagerProfile.post(
	"/profileListMachinesProgramesProcessing",
	jsonParser,
	async (req, res, next) => {
...
FROM magodmis.ncprograms AS n
WHERE (n.PStatus = 'Cutting' OR n.PStatus = 'Processing')
    AND n.Machine = '${req.body.MachineName}'
    AND EXISTS (
        SELECT 1 
        FROM machine_data.magod_process_list AS p 
        WHERE p.ProcessDescription = n.Operation 
...
shiftManagerProfile.post(
	"/OperationMachinesProgramesCompleted",
	jsonParser,
	async (req, res, next) => {
		//console.log('/profileListMachinesProgramesCompleted' , req.body)
		try {
			mchQueryMod(
				`SELECT * FROM magodmis.ncprograms n where Machine = '${req.body.MachineName}' && Operation = '${req.body.Operation}' && PStatus = 'Completed'`,
				(err, data) => {
					if (err) logger.error(err);
					//console.log(data)
					res.send(data);
...
shiftManagerProfile.post(
	"/OperationMachinesProgramesProcessing",
	jsonParser,
	async (req, res, next) => {
		//console.log('/profileListMachinesProgramesProcessing' , req.body)
		try {
			misQueryMod(
				`SELECT * 
FROM magodmis.ncprograms AS n 
WHERE n.Machine = '${req.body.MachineName}' 
    AND n.Operation = '${req.body.Operation}' 
    AND (n.PStatus = 'Cutting' OR n.PStatus = 'Processing');`,
				(err, data) => {
					if (err) logger.error(err);
					//console.log(data)
					res.send(data);
...
shiftManagerProfile.post(
	"/OperationProgramesCompleted",
	jsonParser,
	async (req, res, next) => {
		//console.log('/profileListMachinesProgramesCompleted' , req.body)
		try {
			mchQueryMod(
				`SELECT * FROM magodmis.ncprograms where Operation = '${req.body.Operation}' && PStatus = 'Completed'`,
				(err, data) => {
					if (err) logger.error(err);
					//console.log(data)
					res.send(data);
...
shiftManagerProfile.post(
	"/CustomerProgramesCompleted",
	jsonParser,
	async (req, res, next) => {
		//console.log('/profileListMachinesProgramesCompleted' , req.body)
		try {
			mchQueryMod(
				`SELECT * FROM magodmis.ncprograms where Cust_Code = '${req.body.Cust_Code}' && PStatus = 'Completed' `,
				(err, data) => {
					if (err) logger.error(err);
					//console.log(data)
					res.send(data);
	...
	async (req, res, next) => {
		...
		try {
			misQueryMod(
				`SELECT * 
FROM magodmis.ncprograms AS n 
WHERE n.Cust_Code = '${req.body.Cust_Code}' 
    AND (n.PStatus = 'Cutting' OR n.PStatus = 'Processing');
`,
				(err, data) => {
					if (err) logger.error(err);
...
shiftManagerProfile.post(
	"/OperationProgramesProcessing",
	jsonParser,
	async (req, res, next) => {
		//console.log('/profileListMachinesProgramesProcessing' , req.body)
		try {
			misQueryMod(
				`SELECT * FROM magodmis.ncprograms n where  Operation = '${req.body.Operation}' and  (PStatus = 'Cutting' OR n.PStatus = 'Processing')`,
				(err, data) => {
					if (err) logger.error(err);
					//console.log(data)
					res.send(data);
...
shiftManagerProfile.get(
	...
	async (req, res, next) => {
		...
		try {
			mchQueryMod(
				`SELECT ol.Operation, ol.ProcessId,mpl.Machine_srl, ml.refName, n.NCProgramNo, n.TaskNo , n.PStatus FROM machine_data.profile_cuttingoperationslist pcol
        ...
        LEFT JOIN magodmis.ncprograms n ON n.Machine=ml.refName AND n.Operation=ol.Operation AND (PStatus='Completed' || PStatus='Cutting')`,
				async (err, data) => {
					if (err) logger.error(err);
					...
					data.forEach((item) => {
						...
						const machineIndex = machines.findIndex(
							(machine) => machine.Machine_srl === machine_srl
						);
						...
						if (machineIndex === -1) {
							...
							if (NCProgramNo !== null && TaskNo !== null && PStatus !== null) {
								machines[machineIndex].process.push({
									NCProgramNo,
									TaskNo,
									PStatus,
								});
        ...
        WHERE n.NCProgramNo='${req.body.NCProgramNo}'`,
				(err, data) => {
					if (err) {
						logger.error(err);
						res.status(500).json({
							error: "An error occurred while updating the first table.",
						});
					} else {
...
shiftManagerProfile.post(
	...
	async (req, res, next) => {
		...
		try {
			misQueryMod(
				`UPDATE magodmis.ncprograms SET PStatus = 'Closed' WHERE NCProgramNo = '${req.body.NCProgramNo}';`,
				(err, data) => {
					if (err) logger.error(err);
					//  console.log(data)
					res.send(data);
...
shiftManagerProfile.post(
	...
	async (req, res, next) => {
		...
		try {
			misQueryMod(
				`UPDATE magodmis.ncprograms SET PStatus = 'Mtrl Issue' WHERE NCProgramNo = '${req.body.NCProgramNo}';`,
				(err, data) => {
					if (err) logger.error(err);
					//  console.log(data)
					res.send(data);
...
shiftManagerProfile.post(
	...
	async (req, res, next) => {
		...
		try {
			misQueryMod(
				`UPDATE magodmis.ncprograms SET Machine = '${req.body.NewMachine}' WHERE NCProgramNo = '${req.body.NCProgramNo}';`,
				(err, data) => {
					if (err) logger.error(err);
					//  console.log(data)
					res.send(data);
          ...
          AND m.Working AND p.OperationId=o.OperationID`,
				(err, data) => {
					...
					for (let i = 0; i < data.length; i++) {
						...
						try {
							misQueryMod(
								`SELECT * FROM magodmis.ncprograms where Machine = '${data[i].refName}'`,
								(err, datanew) => {
									if (err) logger.error(err);
									for (let k = 0; k < datanew.length; k++) {
...
shiftManagerProfile.post(
	...
	async (req, res, next) => {
		...
		try {
			mchQueryMod(
				`SELECT n.TaskNo, m1.Mtrl_Code, m1.Operation, m1.NestCount, m1.NoOfDwgs, m1.DwgsNested, m1.PartsNested, m1.TotalParts, m1.Priority, m1.EstimatedTime, SUM(n.qty) AS NoOfSheets
      FROM magodmis.ncprograms n
	...
	try {
      ...
      WHERE magodmis.shiftlogbook.FromTime >= CONCAT('${req.body.Date}', ' 06:00:00')
		...
		mchQueryMod(firstQuery, async (err, data) => {
			...
			const MProcessValues = Array.from(
				new Set(data.map((row) => row.MProcess))
			);
			...
			const secondQuery = `SELECT magodmis.shiftlogbook.MProcess, COALESCE(machine_data.operationslist.Operation, 'Administrative') AS Operation
               ...
               LEFT JOIN magodmis.shiftlogbook ON machine_data.operationslist.ProcessId = magodmis.shiftlogbook.MProcess
                WHERE magodmis.shiftlogbook.MProcess IN (${MProcessValues.map(
									(value) => `'${value}'`
								).join(", ")})`;
			...
			mchQueryMod(secondQuery, (err, operationsData) => {
				...
				const combinedData = data.map((row) => ({
					...row,
					Operation: operationsData.find(
						(opData) => opData.MProcess === row.MProcess
					)?.Operation,
				}));
...
shiftManagerProfile.post("/shiftReport", jsonParser, async (req, res, next) => {
	try {
		mchQueryMod(
			`Select ml.refName, sl.ShiftID,sl.Machine,sl.MProcess,sl.FromTime, sl.ToTime, sl.TaskNo,  sr.Shift, TIMESTAMPDIFF(MINUTE, sl.FromTime, sl.ToTime) as timediff, sl.StoppageID, 
        ...
        from  machine_data.machine_list as ml left join magodmis.shiftlogbook as sl on sl.Machine= ml.refName and DATE(sl.FromTime) like '${req.body.Date}' left  join magodmis.shiftregister as sr on sl.ShiftID=sr.ShiftID`,
			(err, data) => {
				if (err) logger.error(err);
				...
				data.forEach((item) => {
					...
					let machineObj = MachineProcessData.find(
						(machine) => machine.MachineName === item.Machine
					);
					...
					if (item.ShiftID != null) {
						...
						let operationObj = machineObj.operations.find(
							(operation) => operation.Operation === (item.operation || "Break")
						);
						...
						if (!operationObj) {
							...
							operationObj.time = (
								parseInt(operationObj.time) + parseInt(item.timediff)
							).toString();
						}
					}
				});
